...
namespace svMassReco 
{
  ...
  void SVmassRecoSingleLegLikelihoodBase::setPoints(const GlobalPoint& pv, double x, double y, double z, double m12scale, int& error)
  {
  ...
  double SVmassRecoSingleLegLikelihoodBase::nllTopological() const
  {
  ...
  double SVmassRecoSingleLegLikelihoodBase::nllM12Penalty() const 
  {
  ...
  double SVmassRecoSingleLegLikelihoodBase::visRapidity() const
  { 
    return atanh(visP4_.Vect().Dot(legDir_.unit())/visP4_.e()); 
  }

  double SVmassRecoSingleLegLikelihoodBase::nllDecayLength() const 
  {
    double nll = nllTauDecayLengthGivenMomentum(legDir_.r(), p4_.P());
    return nll;
  }

  FourVector SVmassRecoSingleLegLikelihoodBase::visChargedP4() const 
  {
    FourVector output;
    for ( size_t itrk = 0; itrk < nTracks_; ++itrk ) {
  ...
  void SVmassRecoSingleLegLikelihoodBase::printTo(std::ostream& out) const 
  {
    using namespace std;
  ...
  template class SVmassRecoSingleLegLikelihood<pat::Muon>;
  template class SVmassRecoSingleLegLikelihood<pat::Electron>;
  template class SVmassRecoSingleLegLikelihood<pat::Tau>;
  template class SVmassRecoSingleLegLikelihood<reco::Candidate>;
