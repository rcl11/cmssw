...
class PhotonHLTMatching : public edm::EDProducer {
public:
  explicit PhotonHLTMatching(const edm::ParameterSet&);
  ~PhotonHLTMatching();
...
void PhotonHLTMatching::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if(!triggerSummary.isValid()) {
    edm::LogError("HLTMatching/PhotonMatching") << "Missing triggerSummary with label " << triggerSummaryLabel <<std::endl;
    return;
  }

  iEvent.getByLabel(triggerResultsLabel, triggerResults);
  if(!triggerResults.isValid()) {
    edm::LogError("HLTMatching/PhotonMatching") << "Missing triggerResults with label " << triggerResultsLabel <<std::endl;
    return;
  }
...
template <class T1, class T2> 
std::vector<int> PhotonHLTMatching::matchByDeltaR(const std::vector<T1> & collection1, 
...
trigger::TriggerObjectCollection PhotonHLTMatching::selectTriggerObjects(const trigger::TriggerObjectCollection & triggerObjects,
...
reco::PhotonCollection PhotonHLTMatching::selectPhotons(const reco::PhotonCollection& allPhotons, 
...
DEFINE_FWK_MODULE(PhotonHLTMatching);
