...
namespace svMassReco 
{
  ...
  class SVmassRecoDiTauLikelihoodBase : public TObject
  {
   public:
    ...
    /// Access to the legs
    virtual const SVmassRecoSingleLegLikelihoodBase* leg1() const = 0;
    virtual const SVmassRecoSingleLegLikelihoodBase* leg2() const = 0;

    friend std::ostream& operator<< (std::ostream &out, SVmassRecoDiTauLikelihoodBase &fit) { fit.printTo(out); return out; };
   ...
   public:
    /// Each nu-fit has two solutions, corresponding to the nu direction in the rest frame
	 ...
	 m12Limits = 1.0;
       ...
       // can ignore m12
       if ( fixM12ScaleParameter<T1>() ) to.FixParameter(6);
       if ( fixM12ScaleParameter<T2>() ) to.FixParameter(10);
     }
	 ...
	 zLimit = 30.0; //cm
       ...
       // For hadronic taus, there is only 1 nu, so it's mass is always zero and we 
       // can ignore m12
       if ( fixM12ScaleParameter<T1>() ) minuit.FixParameter(6);
       if ( fixM12ScaleParameter<T2>() ) minuit.FixParameter(10);
     }
     ...
     void releaseNuMassScalers(TMinuit& minuit)
     {
       ...
       if ( !fixM12ScaleParameter<T1>() ) minuit.Release(6);
       if ( !fixM12ScaleParameter<T2>() ) minuit.Release(10);
     }
